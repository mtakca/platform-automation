...
The files under `inventories/example/` demonstrate a clean, modular structure containing dummy IP addresses for Load Balancers, Kubernetes Masters, Workers, DBs, and Bastion servers. Sensitive variables and hashes are intended to be encrypted using Ansible Vault (represented as `{{ vault_password }}` in the examples).
